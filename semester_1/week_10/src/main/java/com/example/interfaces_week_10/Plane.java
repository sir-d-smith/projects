...
public class Plane implements Comparable<Plane>
{
    ...
    boolean setCallsign(String callsign)
    {
        ...
        if (callsign.equals("")) { this.callsign = ""; return true; }
    ...
    boolean setFlightNumber(String flight_number)
    {
        ...
        if (flight_number.equals("")) { this.flight_number = ""; return true; }
    ...
    // The compareTo method for this class.
    // It compares two Plane classes and returns
    // an integer based on the input.
    @Override
    public int compareTo(Plane plane)
    {
        int result = this.manufacturer.compareTo(plane.manufacturer);
        if (result == 0)
        {
            result = this.type.compareTo(plane.type);
        }
        if (result == 0)
        {
            result = this.callsign.compareTo(plane.callsign);
        }
        if (result == 0)
        {
            result = this.owner.compareTo(plane.owner);
        }
        if (result == 0)
        {
            result = Boolean.compare(this.is_flying, plane.is_flying);
        }
        if (result == 0)
        {
            result = this.flight_number.compareTo(plane.flight_number);
        }
        // this is not good -- not comparing against the locations!
        // null pointer exceptions kept creeping up at me, too bad!
        // if (result == 0)
        // {
        //     result = this.location.compareTo(plane.location);
        // }
        // if (result == 0)
        // {
        //     result = this.departing_from.compareTo(plane.departing_from);
        // }
        // if (result == 0)
        // {
        //     result = this.arriving_to.compareTo(plane.arriving_to);
        // }
        return result;
    }
}